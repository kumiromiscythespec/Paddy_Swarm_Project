...
BUILDER = Path(__file__).resolve().parents[1] / "build_dual_l_slide_rimless_hub_v0_9_6_15.py"
spec = importlib.util.spec_from_file_location("dual_l_v09615", BUILDER)
...
class DualLSlideRimlessContract(unittest.TestCase):
    maxDiff = None


def _make_test(name: str, ok: bool, detail: object):
    def test(self: DualLSlideRimlessContract) -> None:
...
for index, (name, ok, detail) in enumerate(CHECKS, 1):
    ...
    setattr(DualLSlideRimlessContract, f"test_{index:03d}_{safe}", _make_test(name, ok, detail))
